        ...
        return (
            <div className="space-y-6 pb-20 animate-fade-in">
                <div className="mb-6">
                    ...
                    <p className="text-slate-500 mt-1 text-sm">145 leçons &middot; English Grammar in Use — Raymond Murphy</p>
                </div>
    ...
    if (view === 'units' && selectedCategory) {
        ...
        return (
            <div className="space-y-6 pb-20 animate-slide-in-right">
                <button onClick={goBack} className="flex items-center gap-2 text-slate-500 hover:text-brand-600 font-medium transition-colors text-sm">
                    ...
                    {allUnitsInCategory.map((unitId, idx) => {
                        ...
                        return (
                            <button key={unitId}
                                ...
                                onClick={() => openUnit(u)}
                            >
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm shrink-0 ${completed ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                                    {completed ? <CorrectIcon size={16} /> : unitId}
                                </div>
                                <div className="flex-1">
                                    <div className={`font-medium text-sm ${completed ? 'text-slate-800' : 'text-slate-900'}`}>{u.title}</div>
                                </div>
                                {completed && score !== undefined && (
                                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-md">{score}%</span>
                                )}
                                {!completed && <ArrowRightIcon size={14} className="text-slate-300" />}
                            </button>
                        )
                    })}
                </div>
        ...
        return (
            <div className="space-y-6 pb-24 animate-slide-in-right">
                ...
                <div className="border-b border-slate-200 pb-5 animate-fade-in">
                    <div className="text-brand-600 font-bold uppercase tracking-wider text-xs mb-1">Unit {unit.id}</div>
                    <h2 className="text-2xl font-display font-bold text-slate-900">{unit.title}</h2>
                </div>
                    ...
                    <div className="space-y-6 animate-fade-in" key="cours">
                        ...
                        <section className="animate-reveal-up" style={staggerDelay(0, 100)}>
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h3 className="text-lg font-display font-bold text-slate-900">Leçon</h3>
                            </div>
                        ...
                        {unit.rules && (
                            <section className="animate-reveal-up" style={staggerDelay(1, 100)}>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="text-lg font-display font-bold text-slate-900">Règles clés</h3>
                                </div>
                        ...
                        {unit.examples && (
                            <section className="animate-reveal-up" style={staggerDelay(2, 100)}>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="text-lg font-display font-bold text-slate-900">Exemples</h3>
                                </div>
                        ...
                        {unit.tips && unit.tips.length > 0 && (
                            <section className="animate-reveal-up" style={staggerDelay(3, 100)}>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="text-lg font-display font-bold text-slate-900">Astuces pour francophones</h3>
                                </div>
                                <div className="bg-violet-50/50 p-5 rounded-2xl border border-violet-100 space-y-3">
                                    {unit.tips.map((tip, i) => (
                                        <div key={i} className="flex gap-3 animate-fade-in-up" style={staggerDelay(i, 80)}>
                                            <p className="text-violet-900 text-[15px]">{tip}</p>
                                        </div>
                                    ))}
                                </div>
                            </section>
                    ...
                    <div className="space-y-5 animate-fade-in" key="exercices">
                        ...
                        <div className="space-y-4">
                            {filteredExercises.map((ex, filteredIdx) => {
                                ...
                                return (
                                    <div key={exIdx} className={`bg-white p-5 rounded-2xl border border-slate-200/80 shadow-soft transition-all duration-300 animate-reveal-up ${isLastAnswered && state?.answered ? (state.correct ? 'animate-correct-flash' : 'animate-shake') : ''}`} style={staggerDelay(filteredIdx, 50)}>
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Question {filteredIdx + 1}</span>
                                        ...
                                        <div className="text-base font-medium text-slate-900 mb-4">{ex.question}</div>

                                        {/* Hint */}
                                        {ex.hint && !state?.answered && (
                                            <div className="mb-4 text-sm text-brand-500/80 italic flex items-center gap-1.5">
                            ...
                            {!showResults && allExAnswered && (
                                <div className="flex justify-center pt-2 animate-fade-in-up">
                                    <button className="btn btn-primary px-8 py-3 text-base active:scale-[0.97] transition-transform" onClick={submitExercises}>Valider mes réponses</button>
                                </div>
                            )}
                    ...
                    <div className="space-y-5 animate-fade-in" key="evaluation">
                        ...
                        {!showEvalResults && totalEval > 0 && (
                            <div>
                                ...
                                <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                                    <div className="h-full bg-violet-500 rounded-full transition-all duration-500 ease-out" style={{ width: `${evalProgressPct}%` }} />
                                </div>
                            </div>
                        )}
                        ...
                        <div className="space-y-4">
                            {unit.evaluation.map((ex, evalIdx) => {
                                ...
                                return (
                                    <div key={evalIdx} className={`bg-white p-5 rounded-2xl border border-slate-200/80 shadow-soft transition-all duration-300 animate-reveal-up`} style={staggerDelay(evalIdx, 60)}>
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="text-xs font-bold text-violet-400 uppercase tracking-wider">Évaluation {evalIdx + 1}</span>
                                        ...
                                        <div className="grid gap-2.5">
                                            {(shuffledEvalOptions[`eval-${evalIdx}`] || ex.options).map((opt, optIdx) => {
                                                ...
                                                if (state?.answered) {
                                                    ...
                                                    classes += 'border-slate-200 bg-white text-slate-700 hover:border-violet-300 hover:bg-violet-50/30 cursor-pointer active:scale-[0.98]'
                                                }
                                                return (
                            ...
                            {/* Eval results */}
                            {showEvalResults && (
                                <div className="bg-white rounded-2xl p-8 border border-violet-200 shadow-lift text-center animate-pop">
                                    ...
                                    <p className="text-slate-500 mb-2">Score : {Math.round((evalCorrectCount / totalEval) * 100)}%</p>
